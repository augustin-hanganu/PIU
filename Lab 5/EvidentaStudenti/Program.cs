...
namespace EvidentaStudenti
{
    class Program
    {
        ...
        public static Student CitireStudentTastatura()
        {
            ...
            Console.WriteLine("Introduceti grupa (ex: 1A, 2B):");
            string grupa = Console.ReadLine() ?? "";

            // Cerinta 2: citire ProgramStudiu
            Console.WriteLine($"Introduceti programul de studiu ({string.Join(", ", Enum.GetNames(typeof(ProgramStudiu)))}):");
            string inputProgram = Console.ReadLine() ?? "";
            if (!Enum.TryParse<ProgramStudiu>(inputProgram, ignoreCase: true, out ProgramStudiu programStudiu))
            {
                Console.WriteLine("Program invalid. Se seteaza implicit: Licenta.");
        ...
        {
            Console.WriteLine("Studentii fara note sunt:");
